...
import { UnavailableAgentHostChannel } from './agentHostChannel.js';
...
export async function setupServerServices(connectionToken: ServerConnectionToken, args: ServerParsedArgs, REMOTE_DATA_FOLDER: string, disposables: DisposableStore) {
	...
	// ---- Agent host wiring -------------------------------------------------
	//
	// The bundled agent host has been removed from core, so this server never
	// spawns or bridges to one. The IPC channel is still registered with a stub
	// so renderers get an explicit error rather than `Unknown channel`.
